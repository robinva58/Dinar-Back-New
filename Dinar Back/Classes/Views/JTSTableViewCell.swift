...
class JTSTableViewCell: UITableViewCell {
    ...
    let awakeAnimationInterval = 0.3
    let titleLabelTopConstraintUnhiddenVal = 2.0
    let titleLabelTopConstraintHiddenVal = 24.0
    ...
    func animateTitleLabel(empty:Bool){
        ...
        UIView.animate(withDuration: self.animationInterval) {
            ...
            if(self.mandatoryField == true){
                ...
                self.titleLabel.textColor = self.hasError || empty ? UIColor.titleRedColor() : UIColor.titleGrayColor()
                self.separatorLabel.backgroundColor = self.hasError || empty ? UIColor.titleRedColor() : UIColor.titleGrayColor()
                if(empty == true){
